...
- REST API для приема валидных xAPI Statements (строгая валидация Pydantic по профилю Evidence Locker).
...
- 100% строгая валидация входящих xAPI данных по профилю Evidence Locker (некорректный JSON должен отбрасываться). Для полноценной работы "единого окна" коллекторы нормализуют данные, извлекая важные бизнес-поля.
...
success_metrics:
  ...
  - строгая блокировка невалидного xAPI формата (по профилю Evidence Locker)

risks:
  - необходимость приводить данные к профилю Evidence Locker (с извлечением `source_system` и `context`) может потребовать более сложных адаптеров
